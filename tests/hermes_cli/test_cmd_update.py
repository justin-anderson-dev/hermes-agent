...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        #   1. repo root  — pnpm workspace if pnpm-lock.yaml present, else npm
        ...
        # PROJECT_ROOT uses pnpm when pnpm-lock.yaml is present at the repo
        # root (post-pnpm migration); otherwise it falls back to npm. ui-tui
        # always uses npm.
        pnpm_root_call = (
            ["/usr/bin/pnpm", "install", "--frozen-lockfile"],
            PROJECT_ROOT,
        )
        npm_root_call = (update_flags, PROJECT_ROOT)
        ui_tui_call = (update_flags, PROJECT_ROOT / "ui-tui")
        assert pm_calls[:2] in (
            [pnpm_root_call, ui_tui_call],
            [npm_root_call, ui_tui_call],
        )
        if len(pm_calls) > 2:
            assert pm_calls[2:] == [
        ...
        # to the user. Accept either pnpm or npm at the repo root.
        repo_and_tui_calls = [
            call
            for call in mock_run.call_args_list
            if call.args
            and call.args[0][0] in ("/usr/bin/npm", "/usr/bin/pnpm")
            and call.args[0][1] in ("ci", "install")
        ...
        for call in repo_and_tui_calls:
            assert call.kwargs.get("capture_output") is False, (
                "repo-root / ui-tui install must stream output "
