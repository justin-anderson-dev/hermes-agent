...
| `HERMES_KANBAN_DB` | Pin the kanban database file path directly (highest precedence; beats `HERMES_KANBAN_BOARD` and `HERMES_KANBAN_HOME`). The dispatcher injects this into worker subprocess env so profile workers converge on the dispatcher's board. **Test-isolation hazard:** because this beats `HERMES_HOME`, any tooling that spawns a child `hermes kanban …` subprocess with `env=dict(os.environ)` will leak a parent-shell pin straight to the child — the child writes to the leaked path, not the test's tmp `HERMES_HOME`. Scrub `HERMES_KANBAN_DB` / `HERMES_KANBAN_BOARD` / `HERMES_KANBAN_HOME` / `HERMES_KANBAN_WORKSPACES_ROOT` from the child env when isolating |
